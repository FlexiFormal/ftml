...
pub fn oms<B: SendBackend>(uri: SymbolUri, _in_term: bool, children: ClonableView) -> AnyView {
    ...
    if FtmlConfig::allow_notation_changes() {
        ...
        super::notations::has_notation::<B>(head, children, None)
    } else {
        children.into_view::<crate::Views<B>>()
    }
}
    ...
    if FtmlConfig::allow_notation_changes() {
        match var {
            ...
            Variable::Ref { declaration, .. } => {
                super::notations::has_notation::<B>(declaration.into(), children, None)
            }
        }
    } else {
        children.into_view::<crate::Views<B>>()
...
pub fn oma<B: SendBackend>(
    ...
    let ret = move |children| {
        if let Some(uri) = &uri {
            super::notations::has_notation::<B>(uri.clone(), children, Some(head))
        } else {
            children.into_view::<crate::Views<B>>()
        }
    };
